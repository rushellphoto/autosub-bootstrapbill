...
def getShowidApi(showName):
    ...
    """
    # TO DO:Put show Name in hash table for future referencing
    ...
    if autosub.Helpers.checkAPICallsTvdb(use=True):
        ...
        for sub in dom.getElementsByTagName('Series'):
            #title = sub.getElementsByTagName('SeriesName')[0].firstChild.data.lower()
            # Partial matches are also returned
            #Assume that first match is best
